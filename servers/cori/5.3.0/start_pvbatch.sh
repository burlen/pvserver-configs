...
if [ $# != 6 ]
then
  echo "Usage: start_pvbatch.sh NCPUS WALLTIME ACCOUNT PORT"
  echo
  ...
  echo "  SCRIPT    - full path to the script to run."
  echo
  exit -1
fi
...
echo '             __        __      __       ____   ____  ___ '
echo '   ___ _  __/ /  ___ _/ /_____/ /  ____/ __/  |_  / / _ \'
echo '  / _ \ |/ / _ \/ _ `/ __/ __/ _ \/___/__ \_ _/_ <_/ // /'
echo ' / .__/___/_.__/\_,_/\__/\__/_//_/   /____(_)____(_)___/ '
echo '/_/                                                      '
...
JID=`sbatch -J ParaView-$PV_VER_FULL -A "$ACCOUNT" -p "$QUEUE" --qos=$QOS --nodes=$NNODES -t $WALLTIME -C haswell $PV_HOME/start_pvbatch.qsub`
ERRNO=$?
if [ $ERRNO == 0 ]
then
