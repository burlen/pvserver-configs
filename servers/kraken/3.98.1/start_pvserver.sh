...
export PV_PATH=$PATH
export PV_PYTHONPATH=$PYTHONPATH
export PV_PYTHONHOME=$PYTHONHOME
JID=`qsub -v PV_NCAT_PATH,PV_PORT,PV_NCPUS,PV_NCPUS_PER_SOCKET,PV_RENDER_THREADS,PV_LOGIN_HOST,PV_LOGIN_PORT,PV_LD_LIBRARY_PATH,PV_PATH,PV_PYTHONPATH,PV_PYTHONHOME -N PV-$PV_VER_FULL-$PORT -A $ACCOUNT -q $QUEUE -l size=$SIZE,walltime=$WALLTIME $PV_HOME/start_pvserver.qsub`
ERRNO=$?
if [ $ERRNO == 0 ]
then
