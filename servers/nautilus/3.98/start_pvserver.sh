...
JID=`qsub -S /bin/bash -v MPI_TYPE_MAX -v PV_PATH -v _PATH -v _LD_LIBRARY_PATH -v PV_NCPUS -v PV_PORT -v PV_LOGIN_HOST -v PV_LOGIN_PORT  -N PV-3.98-$PORT -A $ACCOUNT -q $QUEUE -l ncpus=$NCPUS,mem=$MEM\MB,walltime=$WALLTIME /sw/analysis/paraview/3.98/sles11.1_intel11.1.038/start_pvserver.qsub`
ERRNO=$?
if [ $ERRNO == 0 ] 
then
