...
JID=`qsub -S /bin/bash -v MPI_TYPE_MAX,PV_PATH,PATH,LD_LIBRARY_PATH,PV_NCPUS,PV_PORT,PV_LOGIN_HOST,PV_LOGIN_PORT -N PV-3.12.0-$PORT -A $ACCOUNT -q $QUEUE -l ncpus=$NCPUS,mem=$MEM\MB,walltime=$WALLTIME /sw/analysis/paraview/3.12.0/sles11.1_intel11.1.038/start_pvserver.qsub`
ERRNO=$?
if [ $ERRNO == 0 ] 
then
