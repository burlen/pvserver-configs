...
PV_HOME=/usr/common/graphics/ParaView/3.10.0-mom-so

# modules envirtonment isn't setup when using the ssh command.
LD_LIBRARY_PATH=$PV_HOME/lib:$PV_HOME/lib/paraview-3.10:/usr/common/graphics/ParaView/Mesa-7.10.1/lib/:/usr/common/usg/python/2.7.1/lib:/opt/gcc/mpc/0.8.1/lib:/opt/gcc/mpfr/2.4.2/lib:/opt/gcc/gmp/4.3.2/lib:/opt/gcc/4.6.1/snos/lib64:/opt/moab/6.1.4/lib:/opt/cray/atp/1.4.1/lib
...
JID=`qsub -V -N PV-3.10.0-$PORT -A $ACCOUNT -q $QUEUE -l mppwidth=$NCPUS -l mppnppn=$NCPUS_PER_NODE -l walltime=$WALLTIME $PV_HOME/start_pvserver.qsub`
ERRNO=$?
if [ $ERRNO == 0 ]
then
