...
LD_LIBRARY_PATH=$PV_HOME/lib:$PV_HOME/lib/paraview-3.98:$PV_HOME/lib/system-libs/:$MESA/lib:/usr/common/usg/python/2.7.1/lib:/opt/gcc/default/snos/lib64:/opt/gcc/mpc/0.8.1/lib:/opt/gcc/mpfr/2.4.2/lib:/opt/gcc/gmp/4.3.2/lib:/opt/gcc/4.7.1/snos/lib64:/opt/moab/7.2.3-r19/lib
...
JID=`qsub -V -N PV-3.98.1-$PORT -A $ACCOUNT -q $QUEUE -l mppwidth=$NCPUS -l mppnppn=$NCPUS_PER_NODE -l walltime=$WALLTIME $PV_HOME/start_pvserver.qsub`
ERRNO=$?
if [ $ERRNO == 0 ]
then
...
do
  ...
  case $inchar in
    ...
    P|p)
      echo
      less ~/PV-3.98.1-$PORT.e$JIDNO
      echo
      ;;

    L|l)
      echo
      ls -lah ~/PV-3.98.1-$PORT.e$JIDNO
      echo
      ;;
